...
if(empty(_$SESSION['privilege']) || !(_$SESSION['privilege'] == 'store_employee') || !(_$SESSION['privilege'] == 'admin'))
{
...
function local_store_th_id($ls_id){
	$stmt = $dbh->prepare("SELECT th_id FROM  local_stores N ls_employees.ls_id = local_stores INNER JOIN transit_hubs ON transit_hubs.th_id = local_stores.th_id WHERE ls_id = :ls_id" );
	bindParam(':ls_id', $ls_id);
	$stmt->execute() 
	$th_id = stmt->fetch();
	return $th_id;
...
function this_th_id() {
	...
	$stmt = $dbh->prepare("SELECT th_id FROM employees INNER JOIN ls_employees ON employees.id = ls_employees.id INNER JOIN local_stores ON ls_employees.ls_id = local_stores.ls_id INNER JOIN transit_hubs ON transit_hubs.th_id = local_stores.th_id WHERE id = :user_id" );
	bindParam(':user_id', $_SESSION['user_session'] )
	$stmt->execute() 
	$th_id = stmt->fetch();
	return $th_id;
...
function this_ls_id() {
	...
	$stmt = $dbh->prepare("SELECT ls_id FROM employees INNER JOIN ls_employees ON employees.id = ls_employees.id INNER JOIN local_stores ON ls_employees.ls_id = local_stores.ls_id  WHERE id = :user_session");
	bindParam(':user_id', $_SESSION['user_session'] )
	$stmt->execute() 
	$ls_id = stmt->fetch();
	return $ls_id;
...
function create_shipment($shipment_id, $sender, $reciever, $eta, $est_cost, $express) {
	...
	bindParam(':shipment_id', $shipment_id);
	bindParam(':sender', $sender);
	bindParam(':reciever', $reciever);
	bindParam(':this_ls_id', $this_ls_id);
	bindParam(':ls_destination', $ls_destination);
	bindParam(':express', $express);
	bindParam(':est_cost', $est_cost);
	bindParam(':eta', $eta);
...
function order_is_delivered($order_number) {
	...
	bindParam(':order_number', $order_number);
...
function track_order($tracking_number) {
	$stmt = $dbh->prepare("SELECT * FROM track_history where track_no = :tracking_number");
	bindParam(':tracking_number', $tracking_number);
...
function create_trackhistory($tracknumber, $path) {
	...
	foreach($path as $index) {
		$query_vector[$index] "INSERT INTO trackhistory (past_through, tracknumber, transitnode) VALUES (0, :tracknumber, :node )";;
		$query_vector[$index]->bindParam(':tracknumber', $tracknumber);
		$query_vector[$index]->bindParam(':node', $path[$index]);

		// $query_vector[$index]->bindParam(':ETA', $path_cost_matrix[$index][1]);
